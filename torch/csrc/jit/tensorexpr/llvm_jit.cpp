 ...
 public:
  PytorchLLVMJITImpl(
    ...
    // - https://github.com/llvm/llvm-project/issues/65253
    {
      ...
      std::unique_ptr<llvm::Module> module_ = std::make_unique<llvm::Module>("__asan_workaround_fill", *ctx);
      llvm::Type* type = llvm::ArrayType::get(llvm::Type::getInt8Ty(*ctx), 8);
      llvm::Constant* global = module_->getOrInsertGlobal("__asan_workaround_fill", type,
        [&]() {
        return new llvm::GlobalVariable(*module_, type, true, llvm::GlobalVariable::InternalLinkage,
                                        llvm::Constant::getNullValue(type), "__asan_workaround_fill");
      });
      assertSuccess(
        LLJ->addIRModule(ThreadSafeModule(std::move(module_), std::move(ctx))));
    }
  }
