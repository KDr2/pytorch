...
+ std::string(R"=====(def movedim(self: List[int],
  ...
  if torch.le(self_dim, 1):
    ...
    nomralized_src = annotate(List[int], [])
    nomralized_dst = annotate(List[int], [])
    for i in range(torch.len(source)):
      ...
      if torch.lt(_1, 0):
        ...
        dim = _1
      _3 = torch.append(nomralized_src, dim)
      _4 = destination[i]
      if torch.le(self_dim, 0):
        dim_post_expr0 = 1
      else:
      ...
      if torch.lt(_4, 0):
        ...
        dim0 = _4
      _6 = torch.append(nomralized_dst, dim0)
    ...
    for i3 in range(torch.len(source)):
      _10 = nomralized_src[i3]
      _11 = torch._set_item(order, nomralized_dst[i3], _10)
      _12 = torch._set_item(src_dims, nomralized_src[i3], -1)
      _13 = torch._set_item(dst_dims, nomralized_dst[i3], -1)
