...
struct TORCH_API ExecutionTraceObserver {
  ...
  void savePath4GeneratedKernels(
      std::vector<std::string>& kernel_file_paths_in) {
    kernelFilePaths = std::move(kernel_file_paths_in);
  }
...
#ifndef __APPLE__
  // MacOS does not support std::filesystem, also PT2 model does not support
  // MacOS so we skip the copy of generated kernel code to the same directory as
  // the trace file.

  namespace std_fs = std::filesystem;
  // Copy generated kernel code to the same directory as the trace file.
  std_fs::path work_dir = std_fs::path(ob.fileName).parent_path();
  for (auto& cur_path : ob.kernelFilePaths) {
    if (cur_path.empty()) {
      continue;
    }

    std_fs::path src(cur_path);
    std_fs::path kernel_file_name = src.filename();

    std_fs::path dst(work_dir);
    dst += std_fs::path("/");
    dst += kernel_file_name;
    if (!std_fs::copy_file(
            src, dst, std_fs::copy_options::overwrite_existing)) {
      LOG(ERROR) << "Failed to copy generated kernel code '" << cur_path << "'";
    }
  }
#endif
...
void savePath4GeneratedKernels(std::vector<std::string>& kernelFilePaths) {
  VLOG(1) << "savePath4GeneratedKernels()";
  auto& ob = *ObserverManager::get();
  ob.savePath4GeneratedKernels(kernelFilePaths);
}
