  ...
  return [this,
    ...
    auto decode_tensor = [&]() -> TensorMetadata {
      ...
      if (tensor_metadata_it.exhausted()) {
        LOG(WARNING)
            << "Tensor metadata exhaused prematurely. Reported shapes may be innacurate!";
      ...
      for (C10_UNUSED const auto _ : c10::irange(raw_metadata.size_dim_)) {
        if (tensor_size_strides_it.exhausted()) {
          LOG(WARNING)
              << "Expected Tensor Size mismatch with raw Tensor metadata. Reported shapes may be innacurate!";
      ...
      if (raw_metadata.layout_ == at::kStrided) {
        for (C10_UNUSED const auto _ : c10::irange(raw_metadata.stride_dim_)) {
          if (tensor_size_strides_it.exhausted()) {
            LOG(WARNING)
                << "Expected Tensor Strides mismatch with raw Tensor metadata. Reported shapes may be innacurate!";
            return {raw_metadata, sizes, strides};
          }
