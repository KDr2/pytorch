...
void Reducer::all_reduce_bucket(Bucket& bucket) {
  ...
  if (tensor.is_mtia()) {
    ...
    // memory layout. We have to explicitly copy the data back to 1-D gradients.
...
void Reducer::initialize_bucket_views(Reducer::Bucket& bucket) {
  ...
  for (const auto i : c10::irange(bucket.variables.size())) {
    ...
    // create a new tensor on MTIA for now. This part will be modified in the
    // future after the memory layout changes.
