...
def register_run_with_start_end_rng_state_op():
    ...
    def impl_proxy_dispatch_mode(mode, start_rng_state, end_rng_state, op, *args, **kwargs):
        with disable_proxy_modes_tracing():
            out = run_with_start_end_rng_state(start_rng_state, end_rng_state, op, *args, **kwargs)
        proxy_args = pytree.tree_map(mode.tracer.unwrap_proxy, (start_rng_state, end_rng_state, op, *args))
        proxy_kwargs = pytree.tree_map(mode.tracer.unwrap_proxy, kwargs)
        out_proxy = mode.tracer.create_proxy(
    ...
    def impl_fake_tensor_mode(mode, start_rng_state, end_rng_state, op, *args, **kwargs):
        # Skip setting the set_rng_state as it does not work well with fake tensors.
        # And it does not matter for the fake tensor mode.
        with mode:
            return op(*args, **kwargs)
    ...
    def impl_functional(ctx, start_rng_state, end_rng_state, op, *args, **kwargs):
        ...
        with ctx.redispatch_to_next():
            out = run_with_start_end_rng_state(
                unwrapped_start_rng_state, unwrapped_end_rng_state, op, *unwrapped_args, **unwrapped_kwargs
            )
            return ctx.wrap_tensors(out)
