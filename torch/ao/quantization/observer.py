...
class HistogramObserver(UniformQuantizationObserverBase):
    ...
    def upscale_histogram(
        ...
        bin_size = (orig_max - orig_min) / (n_bins * upsample_rate)
        mid_points_histogram = (
            torch.linspace(orig_min, orig_max, n_bins * upsample_rate + 1)[:-1]
            + 0.5 * bin_size
        )
        new_bin_size = (update_min - update_max) / n_bins
        boundaries_new_histogram = torch.linspace(update_min, update_max, n_bins)
        ...
        update_histogram = torch.bincount(
            bucket_assignments, weights=histogram, minlength=n_bins
        )
    ...
    def _combine_histograms(
        ...
        if orig_min == orig_max:
            ...
            transformed_orig_hist = (
                torch.histc(orig_min, bins=n_bins, min=update_min, max=update_max) # type: ignore[arg-type]
                * bin_value
            )
        ...
        transformed_orig_hist = self.upscale_histogram(
            orig_hist, orig_min, orig_max, update_min, update_max, n_bins, upsample_rate
        )
