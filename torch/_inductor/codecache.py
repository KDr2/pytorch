    ...
    def set(
        ...
        if (
            ...
            if torch.version.hip is None or (asm and asm_type):
                ...
                # Map asm_type to valid hash_type for cache system
                # Valid hash_type values: {"amdgcn", "code", "ptx", "spv"}
                # LLVM formats ('ll', 'bc') must use 'code'
...
"""
        ...
        with lock:
                ...
                for kernel_name, value in CudaKernelParamCache.cache.items():
                    ...
                    if (
                        ...
                        if torch.version.hip is None:
                            ...
                            log.debug(
                                    "Compiling multi-arch bundle: kernel=%s, llvm_ir=%s, output=%s",
                                    kernel_name,
                                    asm_file,
                                    cubin_file,
                            )
            ...
            if config.aot_inductor.package_cpp_only:
                ...
                # Just link the object files that already embed kernel binaries
                if config.aot_inductor.emit_multi_arch_kernel and torch.version.hip is None:
