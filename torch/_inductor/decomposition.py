...
def add(x, y):
    if x.is_complex():
        r = x.real + y.real
        i = x.imag + y.imag;
        return torch.where(torch.arange(2, device=x.device, dtype=torch.uint8)==0, r.unsqueeze(-1), i.unsqueeze(-1))
    else:
        return NotImplemented
...
def imag(self):
    assert self.is_complex(), "imag should only be called on a complex tensor"
