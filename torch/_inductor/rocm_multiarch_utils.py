...
import torch
from torch.utils.cpp_extension import ROCM_HOME, _join_rocm_home
...
def get_rocm_compiler() -> str:
    ...
    if not os.path.exists(clang_path):
        raise RuntimeError(
            f"ROCm clang not found at {clang_path}. "
...
def create_multiarch_bundle(
    ...
    try:
        ...
        log.info(f"Created multi-arch bundle: {output_bundle_path} ({bundle_size} bytes)")
        log.info(f"Bundle contains architectures: {list(code_objects.keys())}")
        return True

    except subprocess.CalledProcessError as e:
        log.error(f"Failed to create bundle")
...
def compile_multiarch_bundle_from_llvm_ir(
    llvm_ir_path: str,
    output_bundle_path: str,
    target_archs: Optional[List[str]] = None
) -> bool:
    """
