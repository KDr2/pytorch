...
class ExternKernel(InputsKernel):
    ...
    def process_kernel(  # type: ignore[no-untyped-def]
        ...
        if shape_env := V.fake_mode.shape_env:
            ...
            if isinstance(V.current_node.target, torch._higher_order_ops.effects.WithEffects):
                remove_effect_token_in_path = True
                node_meta_val = node_meta_val[1] # remove the first effect token
            rebind_unbacked(shape_env, V.current_node, example_output, remove_effect_token_in_path)
