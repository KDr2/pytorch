        ...
        **kwargs,
        ...
        with patch.object(
            ...
            **kernel_options,
            ...
            mod = PyCodeCache.load(code, extra=f"{kwargs['BLOCK_M'], kwargs['BLOCK_K'], kwargs['BLOCK_K']}-{num_stages}-{num_warps}")
...
class ExternKernelCaller(ChoiceCaller):
    ...
    def hash_key(self):
        return "-".join([
            self.choice.name,
            *[f"{kwarg}={repr(val)}" for kwarg, val in self.kwargs.items()],
...
class AlgorithmSelectorCache(PersistentCache):
    def __call__(self, choices: List[ChoiceCaller], input_nodes, layout):
        ...
        timings = self.lookup(choices, choices[0].name, repr([self.key_of(x) for x in input_nodes]), autotune)
    ...
    def log_results(name, input_nodes, timings):
        ...
        for choice in top_k:
            ...
            sys.stderr.write(
                f"  {choice.name} {result:.4f}s {best_time/result:.1%}\n"
            )
