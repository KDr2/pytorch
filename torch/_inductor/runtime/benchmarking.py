...
class ProfilerSession:
    ...
    def _start_profiler(self) -> None:
        ...
        cleanup_thread = threading.Thread(target=self._delayed_cleanup, daemon=True)
        cleanup_thread.start()

    def _delayed_cleanup(self) -> None:
    ...
    def record(self, work_fn: Callable[[], None]) -> list["FunctionEvent"]:  # noqa: F821
        ...
        with self._lock:
            ...
            try:
                ...
                self._profiler.step()  # Transition to RECORD
                work_fn()  # Do the actual benchmarking work
                self._profiler.step()  # Transition to RECORD_AND_SAVE -> NONE
...
class ProfilingBenchmarker(Benchmarker):
    ...
    _sessions: dict[str, ProfilerSession] = {}
    _sessions_lock: threading.Lock = threading.Lock()

    @classmethod
    def _get_session(cls, device_type: str) -> ProfilerSession:
        """Get or create a profiler session for the given device type."""
        with cls._sessions_lock:
            if device_type not in cls._sessions:
                cls._sessions[device_type] = ProfilerSession(device_type)
            return cls._sessions[device_type]
