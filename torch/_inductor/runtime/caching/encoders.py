...
def tuned_bmm_params_encoder(
    ...
    *,
    ...
    # First call mm_args to realize the matrices, exactly as done in tuned_bmm (bmm.py:140-142)
    _m, _n, _k, _layout, mat1_realized, mat2_realized = mm_args(
        mat1, mat2, layout=layout, out_dtype=out_dtype
    )

    # Create MMKernelInputs with the realized matrices (bmm.py:146)
    kernel_inputs = MMKernelInputs([mat1_realized, mat2_realized], out_dtype=out_dtype)
...
def tuned_baddbmm_params_encoder(
    ...
    *,
    ...
    # First call mm_args to realize the matrices, exactly as done in tuned_baddbmm (bmm.py:234)
    _m, _n, _k, _layout, mat1_realized, mat2_realized, inp_expanded = mm_args(
        mat1, mat2, inp, layout=layout
    )

    # Create MMKernelInputs with the realized matrices (bmm.py:237-239)
    kernel_inputs = MMKernelInputs(
        [inp_expanded, mat1_realized, mat2_realized],
        scalars=dict(alpha=alpha, beta=beta),
    )
...
def tuned_mm_plus_mm_params_encoder(
    ...
    *,
    ...
    # Create MMKernelInputs with all 4 realized matrices (mm_plus_mm.py:156)
    kernel_inputs = MMKernelInputs(
...
def tuned_int_mm_params_encoder(
    ...
    *,
    ...
    # First call mm_args to realize the matrices, exactly as done in tuned_int_mm (mm.py:562-564)
    _m, _n, _k, _layout, mat1_realized, mat2_realized = mm_args(
        mat1, mat2, layout=layout, out_dtype=torch.int32
    )

    # Create MMKernelInputs with the realized matrices (mm.py:583)
...
def tuned_scaled_mm_params_encoder(
    ...
    # First call mm_args to realize the matrices (mm.py:963-965)
    ...
    # Create MMKernelInputs (mm.py:979-981)
    kernel_inputs = MMKernelInputs(
