...
class Memoizer(_BaseMemoizer):
    ...
    def _load_cache_from_dump(self) -> None:
        ...
        if entries_to_load:
            if self._sub_key:
                print(f"Loaded {len(entries_to_load)} cache entries from {dump_file_path} (sub_key={self._sub_key})")
            else:
                print(f"Loaded {len(entries_to_load)} cache entries from {dump_file_path}")
    ...
    def _dump_cache_to_json(self) -> None:
        ...
        for key, value in self._cache._memory.items():
            ...
            encoded_params, encoded_result = value
            formatted_cache[key] = {
...
class PersistentMemoizer(_BaseMemoizer):
    ...
    def __init__(self, sub_dir: PathLike[str] | None = None) -> None:
        ...
        self._memoizer: Memoizer = Memoizer(sub_key=self._sub_dir if self._sub_dir else None)
