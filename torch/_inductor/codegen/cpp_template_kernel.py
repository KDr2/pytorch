...
class CppTemplateKernel(CppKernel):
    ...
    def store_output(
        self,
        dst: ir.Buffer,  # tile_Y -> Y + reindex (reindex in microgemm) -> tmp20.store(Y + (x0 * ldy + x1), static_cast<int64_t>(16));
        src: ir.Buffer,  # acc_slice -> C/C_pre (names in microgemm) + reindex (reindex in microgemm) at::vec::Vectorized<int32_t>::loadu(C_pre + (x0 * ldc + x1), static_cast<int64_t>(16));
        orig_src: Optional[ir.Buffer] = None,  # GemmOut -reindex
