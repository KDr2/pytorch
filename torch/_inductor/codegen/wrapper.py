...
class WrapperCodeGen(CodeGen):
    ...
    def generate_user_defined_triton_kernel(
        ...
        self.writeline(
            f"handle = _record_function_with_args_enter(\"{kernel_name}\", {kernel_name}.__file__, {', '.join(args)}, grid=\"{grid}\", stream={stream_name})"
        )
        self.writeline(
    ...
    def generate_kernel_call(
        ...
        if cuda:
            ...
            if triton:
                ...
                self.writeline(
                    f'handle = _record_function_with_args_enter("{name}", {name}.__file__, {call_args_str}, grid="{grid_str}", stream={stream_name})'
                )
                self.writeline(
