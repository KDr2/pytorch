...
from .cpp import KernelGroup, CppKernelProxy
        ...
        def arg_hook():
            ...
            for input_name in get_kernel_input_names(epilogue_nodes):
                if input_name in self.fake_buffers_name:
                    arg = next(
                        (buf for buf in self.fake_buffers if buf.get_name() == input_name),
                        None,
                    )
                else:
                    arg = V.graph.get_buffer(input_name)
                ...
                if len(arg.get_size()) == 2:
                    ...
                    arg_ptr = f"&({kernel.index(arg_tb, [offsets[0], offsets[1]])})"
