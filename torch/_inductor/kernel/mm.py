...
blockwise1x128_scaling = r"""
@triton.jit
def blockwise1x128_scaling(
    pid,
    scale,
    ki,
    lhs_size,
    ...
    scale_expanded = tl.broadcast_to(
        scale_expanded,
        (BLOCK_lhs, (BLOCK_K + 128 - 1) // 128, MIN_BLOCK_TILE_K)
    )
...
{{def_kernel("A", "B", "A_inverse_scale", "B_inverse_scale")}}
    ...
    for _ in range(0, k_tiles * tiles_per_SM):
        ...
        m_blocks = tl.cdiv(M, 128)
        n_blocks = tl.cdiv(N, 128)
        k_blocks = tl.cdiv(K, 128)

        if SCALE_RECIPE_A == 2:  # ScalingType.Blockwise1x128
            scale_a_block = blockwise1x128_scaling(
                pid_m,
                a_scale,
                ki,
                M,
        ...
        elif SCALE_RECIPE_A == 3:  # ScalingType.Blockwise128x128
        ...
        if SCALE_RECIPE_B == 2:  # ScalingType.Blockwise1x128
            scale_b_block = blockwise1x128_scaling(
                pid_n,
                b_scale,
                ki,
                N,
        ...
        elif SCALE_RECIPE_B == 3:  # ScalingType.Blockwise128x128
...
def _is_blockwise1x128_scaling(sz: Any, tensor_sz: Any, transpose: bool) -> bool:
    ...
    return V.graph.sizevars.statically_known_equals(
        ...
        sz[rhs], ceildiv(tensor_sz[rhs], 128)
    )
...
def is_desired_scaling(
    ...
    match scaling_type:
        ...
        case ScalingType.BlockWise1x128:
            return _is_blockwise1x128_scaling(scale_size, t.get_size(), transpose)
    ...
    if (
        ...
        if use_triton_tma_template(mat_a, mat_b, output_layout=layout) and not bias:
            ...
            if (
                ...
                raise AssertionError(
                    "Inductor Triton does not support scaling options that are present in both epilogue scaling and main loop scaling"
                )
