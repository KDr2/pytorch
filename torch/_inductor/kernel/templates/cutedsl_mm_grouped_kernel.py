...
class GroupedGemmKernel:
    ...
    def _setup_attributes(self):
        ...
        # TODO(shikaili): Futher tune configurations.
        mma_inst_tile_k = 8
        self.mma_tiler = (
            self.mma_tiler[0],
            self.mma_tiler[1],
...
@cute.kernel
def _prepare_metatadata_device(
    # Input
    ...
    block_sizes: tuple[cutlass.Constexpr[int], cutlass.Constexpr[int]],
    # Output
    ...
    if tidx == 0:
        ...
        block_size_m, block_size_n = block_sizes

        # TODO(shikaili): Cannot adds offsets to pointers.
        # CuTeDSL doesn't expose dtype control on tensor pointers.
        # Example:
        # note: unknown: see current operation: %16 = "builtin.unrealized_conversion_cast"(%15) : (i64) -> i32
        ...
        if cutlass.const_expr(input_ptr_c.dtype == cutlass.Float32):
            element_size_c = 4
        else:
            element_size_c = 2
        ...
        for g in range(num_groups):
            ...
            if cutlass.const_expr(problem_type == _FPROP):
                ...
                off_a += chunk_size * stride_am  # K major. M stride.
                off_b += N * stride_bn  # K major. N stride.
                off_c += chunk_size * stride_cm  # N major. M stride.
                ...
                off_a += chunk_size * stride_am  # K major. M stride.
                off_b += K * stride_bk  # N major. K stride.
                off_c += chunk_size * stride_cm  # N major. M stride.
            else:
                m = M
                n = N
                ...
                off_a += chunk_size * stride_ak  # M major. K stride.
                off_b += chunk_size * stride_bk  # N major. K stride.
                off_c += M * stride_cm  # N major. M stride.
...
def _prepare_metatadata_host(
    ...
    input_ptr_b: cute.Pointer,
    input_ptr_c: cute.Pointer,
    block_sizes: cutlass.Constexpr[tuple[int, int]],
    # Output
    output_problem_sizes_mnkl: cute.Tensor,
    output_strides_abc: cute.Tensor,
    output_ptrs_abc: cute.Tensor,
    ...
    _prepare_metatadata_device(
        problem_type=problem_type,
        split_sizes=split_sizes,
        input_problem_sizes_gmnk=input_problem_sizes_gmnk,
        input_strides_abc=input_strides_abc,
        input_ptr_a=input_ptr_a,
        input_ptr_b=input_ptr_b,
        input_ptr_c=input_ptr_c,
        block_sizes=block_sizes,
        output_problem_sizes_mnkl=output_problem_sizes_mnkl,
        output_strides_abc=output_strides_abc,
        output_ptrs_abc=output_ptrs_abc,
        output_total_num_clusters=output_total_num_clusters,
    ).launch(
        grid=(num_groups, 1, 1),
        block=(32, 1, 1),
    )
