...
def normalize_full_default_aten(match: Match, *args, **kwargs):
    ...
    with graph.inserting_after(full_node):
        new_full_node = graph.call_function(
            ...
            args=(tensors,),
            kwargs={"fill_value": fill_value, "dtype": dtype, "layout": layout, "device": device, "pin_memory": pin_memory},
        )
...
def decompose_full_cat_aten(match: Match, *args, **kwargs):
    graph = match.graph
    full_node = next(node for node in match.nodes if node.target == torch.ops.aten.full.default)
    # get cat node
    cat_node = next(node for node in full_node.users.keys() if node.target == torch.ops.aten.cat.default)
    # check the inputs of the cat node should either be full or slice node
    if not all(
        node.target in {torch.ops.aten.full.default, torch.ops.aten.slice.Tensor} for node in cat_node.args[0]
    ):
        return
    cat_dim = get_arg_value(cat_node, 1, "dim")
    # iterate over the cat node's inputs and find consecutive full nodes
    decomposed_cat_inputs, sub_inputs = [], []
