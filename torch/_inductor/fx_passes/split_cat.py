...
def move_view_after_cat(match: Match, *args, **kwargs):
    split_node = next(node for node in match.nodes if node.target == torch.ops.aten.split_with_sizes.default)
    split_input, split_section, split_dim = _get_split_args_default(split_node) 
    split_users = list(split_node.users.keys())
    getitem_indices = [getitem.args[1] for getitem in split_users if getitem.target == operator.getitem]
    if not is_sorted_and_consecutive(getitem_indices):
        return
    cat_nodes = [node for node in match.nodes if node.target == torch.ops.aten.cat.default]
    graph = match.graph
