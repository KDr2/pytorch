...
def decompose_select_view_cat_aten(match: Match, *args, **kwargs):
    graph = match.graph
    cat_node = next(node for node in match.nodes if node.target == torch.ops.aten.cat.default)
    cat_dim = get_arg_value(cat_node, 1, "dim")
    view_nodes = list(cat_node.args[0])
    # check the input of the cat node should be view nodes
    if not all(view_node.target == torch.ops.aten.reshape.default for view_node in view_nodes):
        return
    ...
    if not all(select_node.target == torch.ops.aten.select.int for select_node in select_nodes):
        return
    ...
    if not all(slice_node.target == torch.ops.aten.slice.Tensor for slice_node in slice_nodes):
        return
    ...
    if not is_node_meta_valid(slice_nodes[0]) or not is_node_meta_valid(view_nodes[0]) or not is_node_meta_valid(cat_node):
        return
    if math.prod(view_nodes[0].meta["val"].shape) * len(view_nodes) != math.prod(list(slice_nodes[0].meta["val"].shape)):
        return
    
    permute_node = None
    # if cat_dim is not the same as the select_dim, we need to do permute first
    if cat_dim != select_dims[0]:
        permute_index_lst = [i for i in range(len(slice_nodes[0].meta["val"].shape))]
        permute_index_lst[cat_dim], permute_index_lst[select_dims[0]] = select_dims[0], cat_dim
        ...
        with graph.inserting_after(permutation_input):
            ...
            # update meta data
            permute_node.meta["val"] = torch.ops.aten.permute.default(slice_nodes[0].meta["val"], permute_index_lst)
