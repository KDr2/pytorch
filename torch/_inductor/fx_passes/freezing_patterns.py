...
def _register_int4_mm_fusion_patterns(device, val):
    ...
    int4_weight = functools.partial(
        torch.empty, (8, 1, 32, 4), device=device, dtype=torch.int32, requires_grad=False
    )
    int4_scale_zeros = functools.partial(
        torch.empty, (1, 64, 2), device=device, dtype=torch.bfloat16, requires_grad=False
    )
    ...
    def check_int4_gpu_concat_weights(match):
        ...
        for sz in scale_inputs[1:]:
            ...
            if sz_val.shape[0] != first_sz.shape[0] or sz_val.shape[2] != first_sz.shape[2]:
                return False
    ...
    for gs in [32, 64, 128, 256]:
        ...
        register_replacement(
            ...
            [val(), int4_weight(), int4_weight(), int4_weight(),
             int4_scale_zeros(), int4_scale_zeros(), int4_scale_zeros()],
            fwd_only,
            pass_patterns[0],
        ...
        register_replacement(
            ...
            [val(), int4_weight(), int4_weight(),
             int4_scale_zeros(), int4_scale_zeros()],
            fwd_only,
            pass_patterns[0],
            extra_check=check_int4_gpu_concat_weights,
