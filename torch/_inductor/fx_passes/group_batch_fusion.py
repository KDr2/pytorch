...
class PostGradBatchLinearFusion(BatchFusion):
    ...
    def match(self, node: torch.fx.Node) -> Optional[Tuple[str, int, int, int, bool]]:
        if CallFunctionVarArgs(aten.mm).match(node) and self._is_mm_node_input_2d(node):
            input_m, weight_m = node.args
            bias_m = None

        elif CallFunctionVarArgs(aten.addmm.default).match(node) and \
            self._addmm_node_can_be_fused(node) and \
            self._is_mm_node_input_2d(node):
...
def group_batch_fusion_passes(graph: torch.fx.Graph, pre_grad=True):
    ...
    if pre_grad:
        ...
        fbgemm_fusion_keys = [x for x in config.post_grad_batch_fusion_options \
            if config.post_grad_batch_fusion_options[x].get('require_fbgemm', False)]
        ...
        fusions += generate_fusion_from_config(non_fbgemm_fusions)
        if has_fbgemm:
            fusions += generate_fusion_from_config(fbgemm_fusions)
