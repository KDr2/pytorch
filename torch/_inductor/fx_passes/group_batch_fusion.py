    ...
    def match(self, node: torch.fx.Node):
        if CallFunctionVarArgs(self.op).match(
            ...
            if hasattr(input, "meta"):
                ...
                shape = other.meta["tensor_meta"].shape
            else:
    ...
    def fuse(self, graph: torch.fx.GraphModule, subset: List[torch.fx.Node]):
        ...
        for node in subset:
            ...
            if not isinstance(input, torch.fx.Node) and not isinstance(
                ...
                input = convert_scalar_to_tensor(graph, node, input, torch.Size([1, 1]))
                other = convert_scalar_to_tensor(graph, node, other, torch.Size([1, 1]))
            elif not isinstance(input, torch.fx.Node):
                input = convert_scalar_to_tensor(
                    graph, node, input, other.meta["tensor_meta"].shape
                )
            elif not isinstance(other, torch.fx.Node):
                other = convert_scalar_to_tensor(
                    graph, node, other, input.meta["tensor_meta"].shape
                )
