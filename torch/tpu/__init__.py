...
from torch.tpu._tensor import TPUTensor
...
def empty_strided(
    ...
    JAX arrays are always contiguous, so the stride parameter is accepted
    for API compatibility but not enforced.

    Args:
        size: Shape of the tensor.
        stride: Stride of the tensor (accepted but not enforced).
        dtype: PyTorch dtype (default: torch.float32).
...
def rand_strided(
    ...
    for benchmarking compiled Pallas kernels. Stride is accepted for API
    compatibility but not enforced (JAX arrays are always contiguous).
