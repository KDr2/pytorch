        ...
        for pass_name in inductor_config.post_grad_fusion_options:
            if pass_name == "activation_quantization_aten_pass":
                ...
                if torch._inductor.config.post_grad_fusion_options[
                    ...
                    saved_values_names = {node.name: node for node in saved_values if "primals" not in node.name}
                ...
                for fwd_node in fwd_module_outputs:
                    ...
                    if quant_fwd_output:
                        ...
                        with bwd_module.graph.inserting_after(bwd_input):
                            quant_bwd_input = bwd_module.graph.placeholder(name=quant_node.name)
...
def should_quantize(node: torch.fx.Node) -> bool:
    ...
    if (
        not is_node_meta_valid(node)
        or not node.meta["val"].dtype in allowed_dtypes
    ):
        return False
...
def calculate_quantization_scaling(
    graph: torch.fx.Graph, node: torch.fx.Node, max: float = 57344.0, min: float = 1e-12, quant_type: torch.dtype = torch.float8_e5m2
):
    ...
    with graph.inserting_after(amax_64_node):
        ...
        clamp_min_node.meta["val"] = torch.ops.aten.clamp_min.default(amax_64_node.meta["val"], min)
    ...
    with graph.inserting_after(reciprocal_node):
        ...
        mul_node.meta["val"] = torch.ops.aten.mul.Tensor(reciprocal_node.meta["val"], max)
...
def perform_quantization(
    ...
    with graph.inserting_after(scaled_target_node):
        ...
        clamp_min_scaled_node.meta["val"] = torch.ops.aten.clamp_min.default(scaled_target_node.meta["val"], clamp_min)
    ...
    with graph.inserting_after(clamp_min_scaled_node):
        ...
        clamp_max_scaled_node.meta["val"] = torch.ops.aten.clamp_max.default(clamp_min_scaled_node.meta["val"], clamp_max)
...
def quantize_activation_fw(match: Match, *args, **kwargs):
    ...
    for node in fwd_outputs:
        ...
        if node.meta.get("saved_for_quantization", False):
            ...
            if torch._inductor.config.post_grad_fusion_options[
                ...
                scale_node = calculate_quantization_scaling(graph, node, clamp_max, 1e-12, quant_type)
                # converting to fp8
                quant_node = perform_quantization(graph, node, scale_node, quant_type, clamp_min, clamp_max)
                ...
                with graph.inserting_after(node):
                    ...
                    quant_node.meta["val"] = torch.ops.prims.convert_element_type.default(node.meta["val"], quant_type)
                    quant_node.meta["tensor_meta"] = extract_tensor_metadata(quant_node.meta["val"])
    ...
    if scale_nodes:
        output_updated_args = output_updated_args[:idx] + scale_nodes + output_updated_args[idx:]
        ...
        if node.meta.get("saved_for_quantization", False):
            ...
            if torch._inductor.config.post_grad_fusion_options[
                ...
                with graph.inserting_after(node):
                    ...
                    scale_node = [bwd_input for bwd_input in bw_inputs if bwd_input.name == scale_name][0]
                    activation_node = graph.call_function(
                        torch.ops.prims.convert_element_type.default,
                        args=(node, dequant_type),
                    )
                    activation_node.meta["val"] = torch.ops.prims.convert_element_type.default(
                        node.meta["val"], dequant_type
                    )
                    activation_node.meta["tensor_meta"] = extract_tensor_metadata(activation_node.meta["val"])
                with graph.inserting_after(scale_node):
                    divided_target_node_32 = graph.call_function(
                        torch.ops.aten.div.Tensor,
                        args=(activation_node, scale_node),
                    )
                    divided_target_node_32.meta["val"] = torch.ops.aten.div.Tensor(activation_node.meta["val"], scale_node.meta["val"])
                ...
                with graph.inserting_after(node):
                    ...
                    dequant_node.meta["val"] = torch.ops.prims.convert_element_type.default(node.meta["val"], dequant_type)
                    dequant_node.meta["tensor_meta"] = extract_tensor_metadata(dequant_node.meta["val"])
