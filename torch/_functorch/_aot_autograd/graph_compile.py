...
def _aot_stage2b_inference_compile(
    ...
    """
    Compile the inference graph. Returns:
    - the output strides of the inference graph
    - the compiled inference function

    Note that while this should be morally similar to _aot_stage2b_fw_compile,
    there are some differences that may be incidental but worth recording:
    instead of running pre_compile and post_compile for wrappers here, we run
    them in aot_stage2b_inference_compile, and in somewhat different order, and
    with some caching in between.

    Before calling this function, we run pre_compile for the following wrappers:
    - FakifiedOutWrapper
    - FunctionalizedRngRuntimeWrapper
    After calling this function, we run post_compile for the following wrappers:
    - FunctionalizedRngRuntimeWrapper
    - FakifiedOutWrapper
    - EffectTokensWrapper
    - AOTDispatchSubclassWrapper
    ...
    with context(), track_graph_compiling(aot_config, "inference"):
        ...
        return fwd_output_strides, compiled_fw
...
def aot_stage2_inference(
    ...
    fwd_output_strides, compiled_fw = _aot_stage2b_inference_compile(
        fw_module,
