...
Value* symbolicToValue(
  ...
  switch (arg.tag()) {
    ...
    case torch::_export::Argument::Tag::AS_NESTED_TENSORS: {
      ...
      // Create inner ListPack nodes for each inner list, then pack them together
      ...
      for (const auto& innerList : arg.get_as_nested_tensors()) {
        ...
        auto innerListPack =
            graph.createListPack(std::move(innerListValues), Type::Kind::Tensor);
        outerListValues.push_back(
            graph.insertBefore(innerListPack, insertBefore)->outputs()[0]);
      }
      auto outerListPack =
          graph.createListPack(std::move(outerListValues), Type::Kind::TensorList);
