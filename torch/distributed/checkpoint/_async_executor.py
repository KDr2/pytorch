...
class _AsyncCheckpointProcess:
    def __init__(
        ...
        self._mp_manager: SyncManager = self.ctx.Manager()
        self._response_dict: DictProxy[Any, Any] = self._mp_manager.dict()
    ...
    def _checkpointing_subprocess(
        ...
        try:
            ...
            logger.error(
                f"Checkpoint background process encountered an exception during init: {e}"
            )
        ...
        try:
            while True:
                ...
                logger.info(
                    f"Received async checkpoint request with id={obj.checkpoint_request_id.checkpoint_id}"
                )
                ...
                logger.info(
                    f"Submitted checkpoint save request for checkpoint_id={obj.checkpoint_request_id}"
                )
