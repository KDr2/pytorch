...
def _maybe_adjust_transform_info_inplace(
    ...
    """post adjust transforms info"""
    new_transforms_infos = []
    updated_placements: list[Optional[Placement]] = [
        None for _ in range(device_mesh.ndim)
    ]
    for transform_info in transform_infos:
        i = transform_info.mesh_dim
        current, target = transform_info.src_dst_placements
        if updated_placements[i] is not None:
        ...
        if ignore_partial_to_replicate:
            ...
            # adjust rule 4: we should not see Shard -> Partial in forward
...
def _get_ending_placement(
    src_placement: Sequence[Placement], transform_infos: list[_TransformInfo]
) -> list[Placement]:
    end_placement = list(src_placement[:])
    for transform_info in transform_infos:
        i = transform_info.mesh_dim
        _, target = transform_info.src_dst_placements
        end_placement[i] = target
...
def redistribute_local_tensor(
    ...
    *,
    ...
    _maybe_adjust_transform_info_inplace(
        transform_infos, device_mesh, ignore_partial_to_replicate=is_backward
    )
    end_placements = _get_ending_placement(current_spec.placements, transform_infos)
