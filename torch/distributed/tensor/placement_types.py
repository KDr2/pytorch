...
@torch.library.custom_op("_dtensor::scale_tensor_for_partial", mutates_args=())
def _scale_tensor_for_partial(tensor: torch.Tensor, local_rank: int) -> torch.Tensor:
    if local_rank == 0:
        return tensor.clone()
    return torch.zeros_like(tensor)


@_scale_tensor_for_partial.register_fake
def _(tensor: torch.Tensor, local_rank: int) -> torch.Tensor:
    return tensor.clone()


@maybe_run_for_local_tensor
def _call_scale_tensor_for_partial(
    tensor: torch.Tensor, local_rank: int
) -> torch.Tensor:
    return torch.ops._dtensor.scale_tensor_for_partial(tensor, local_rank)
    ...
    def _partition_value(
        ...
        * "band"/"bor"/"bxor": Bitwise operations have no well-defined inverse
        ...
        if self.reduce_op == "sum":
            ...
            return _call_scale_tensor_for_partial(tensor, local_rank)
