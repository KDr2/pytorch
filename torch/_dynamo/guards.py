...
class GuardBuilder(GuardBuilderBase):
    ...
    def NN_MODULE_HOOKS_IDENTITY(self, guard: Guard) -> None:
        ...
        This allows reusing compiled code when different modules have the same
        hook functions, reducing recompilations from O(num_modules) to O(num_unique_hook_patterns).
        """
        if config.skip_nnmodule_hook_guards:
            return
        ...
        # Guard on the identity of each hook function (by their id)
        # The tuple of hook function ids uniquely identifies the hook pattern
        hook_ids = tuple(id(hook) for hook in hooks_dict.values())

        code = [f"tuple(id(v) for v in {ref}.values()) == {hook_ids!r}"]
        self._set_guard_export_info(guard, code)

        # Add ID match guards for each hook function value in the dict
