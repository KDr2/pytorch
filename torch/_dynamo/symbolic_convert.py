...
class InstructionTranslatorBase(
    ...
    def _is_exception_construction_context(self) -> bool:
        """Check if remaining bytecode leads to RAISE_VARARGS without jumps.

        This is used to detect when we're constructing an exception message
        that will be raised. In such cases, if tracing fails, we should try
        to evaluate the exception in eager mode and raise it directly.
        """
        ip = self.instruction_pointer
        if ip is None:
            return False
        for i in range(ip, min(ip + 20, len(self.instructions))):
            inst = self.instructions[i]
            if inst.opname == "RAISE_VARARGS":
                return True
            # Stop at jumps or other control flow that might skip the raise
            if inst.opname in (
                "JUMP_FORWARD",
                "JUMP_BACKWARD",
                "JUMP_ABSOLUTE",
    ...
    def step(self) -> bool:
        ...
        try:
            ...
            # If we're in exception construction context (i.e., heading towards
            # RAISE_VARARGS), try to evaluate the exception in eager mode and
            # raise it instead of the tracing error. This ensures user exceptions
            # are surfaced instead of dynamo tracing errors.
            ...
            if (
                self._is_exception_construction_context()
                and isinstance(e, Unsupported)
