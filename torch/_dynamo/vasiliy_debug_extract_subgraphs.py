...
summary_headers = ['orig_node_name', 'subgraph_idx', 'input_shape', 'lin1_shape', 'lin2_shape', 'subgraph_summary']
    ...
    if hasattr(torch_fn, '__name__'):
        if hasattr(torch, torch_fn.__name__):
            if getattr(torch, torch_fn.__name__) == torch_fn:
                return f'torch.{torch_fn.__name__}'
    return torch_fn
    ...
    * save it to disk for further debugging
    ...
    for node in new_g.nodes:
        if node.op != 'placeholder':
            continue
        printme(f'placeholder {node.format_node()}')
...
def debug_linears_for_float8(
    ...
    Format of summary.csv (column: example_value):
      ...
      subgraph_summary: <class 'torch.nn.modules.activation.ReLU'> -> Linear -> ["<class 'torch.nn.modules.activation.ReLU'>"]
