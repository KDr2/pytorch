...
def _lift_buffers_to_user_inputs(
    ...
    new_node_names = {k: v for k, v in reversed(new_node_names.items())}
...
def _export_non_strict(
    ...
    *,
    ...
    flat_args = pytree.tree_leaves(combine_args_kwargs(fake_args, fake_kwargs))
    index = 0
    ...
    if not strict:
        ...
        def _tuplify_outputs(aot_export):
            def _aot_export_non_strict(mod, args, **kwargs):
                ...
                strip_root = lambda x: x[len('_export_root.'):] if x.startswith('_export_root.') else x
