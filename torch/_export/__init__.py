...
from torch._C._aoti import AOTIModelContainerRunnerCpu, AOTIModelContainerRunnerCuda
...
def aot_load(so_path: str, device: str) -> Callable:
    ...
    Returns:
        A callable
    """
    runner: Union[AOTIModelContainerRunnerCpu, AOTIModelContainerRunnerCuda]
    if device == "cpu":
        runner = AOTIModelContainerRunnerCpu(so_path, 1)  # type: ignore[call-arg]
    elif device == "cuda" or device.startswith("cuda:"):
        runner = AOTIModelContainerRunnerCuda(so_path, 1)  # type: ignore[call-arg]
    else:
        raise RuntimeError("Unsupported device " + device)

    def optimized(*args):
        call_spec = runner.get_call_spec()  # type: ignore[union-attr]
        in_spec = pytree.treespec_loads(call_spec[0])
        out_spec = pytree.treespec_loads(call_spec[1])
        flat_inputs = fx_pytree.tree_flatten_spec((*args, {}), in_spec)
        flat_outputs = runner.run(flat_inputs)  # type: ignore[union-attr]
