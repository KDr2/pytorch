...
def get_custom_op_profiles(
    gm: torch.fx.GraphModule, ops_to_guard: list[str]
...
def generate_and_register_fake_kernels(op_profiles: dict[str, set[OpProfile]]) -> None:
    ...
    def _generate_res(
        ...
        def _generate_tensor_out(t: TensorMetadata) -> torch.Tensor:
            fake_shape = [
                torch._library.fake_impl.allocate_size(fake_mode.shape_env)
            ...
            for i in range(t.rank):
                fake_strides[i] = fake_stride
                fake_stride = fake_stride * fake_shape[i]
        ...
        def _(*args, **kwargs):  # type: ignore[no-untyped-def]
            ...
            for profile in profiles:
                if _match_args(profile.args_profile, (*args, *kwargs.values())):
                    return _generate_res(fake_mode, profile.out_profile)

            raise RuntimeError(
                f"No fake kernel found for {op_name}, and there were no previous "
                "operator profiles that were recorded which matched these "
