...
class ModuleFrame:
    def __init__(
        ...
        if signature is not None and self.parent is not None:
            ...
            self.parent_call_module.args = arg_nodes
        ...
        if signature is not None and self.parent is not None:
            for output in signature.outputs:
                if isinstance(output, (TensorArgument, SymIntArgument)):
                    ...
                    raise RuntimeError(
                        f"Unsupported data type for output node: {output.kind}"
                    )
