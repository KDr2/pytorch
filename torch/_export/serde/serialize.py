    ...
    if has_constexprs:
        ...
        for sig_key, cache_entry in cache.items():
            constexpr_matches = re.findall(
                r"\('constexpr',\s*(-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\)", sig_key
            )
    ...
    if is_autotuner:
        for sig_key, cache_entry in matching_entries:
            ...
            for config in kernel.configs:
                ...
                if hasattr(config, "num_cpu_threads") and hasattr(
                    entry_metadata, "num_cpu_threads"
                ):
                    if config.num_cpu_threads != entry_metadata.num_cpu_threads:
                ...
                if hasattr(config, "num_ctas") and hasattr(entry_metadata, "num_ctas"):
