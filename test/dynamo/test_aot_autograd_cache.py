...
class AOTAutogradCacheTests(InductorTestCase):
    ...
    def test_triton_op_kernel_detection_logging(self):
        """
        Test that get_inner_triton_kernels batches debug log messages and
        silently skips Python builtins and function parameter names rather
        than emitting one log line per name.
        """
        import logging
        ...
        # Builtins and function parameters should NOT appear in any log
        # message — they are silently skipped before logging.
        builtin_names = {"torch", "len", "int", "range", "getattr", "isinstance"}
        param_names = {"x"}
        all_messages = " ".join(r.getMessage() for r in log_records)
        for name in builtin_names | param_names:
            self.assertNotIn(
                f" {name},",
                f" {all_messages},",
                f"builtin/param '{name}' should be silently skipped, not logged",
            )
        ...
        # The old code would produce O(N) messages; the new code produces O(1).
