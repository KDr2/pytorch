...
class CPUReproTests(TestCase):
    ...
    def test_masked_handle_scalar_var(self):
        def fn():
            ...
            masked_window = img_mask.view(1, height // window_size, window_size, width // window_size, window_size, 1)
            masked_window = masked_window.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, window_size * window_size)

            attn_mask = masked_window.unsqueeze(1) - masked_window.unsqueeze(2)
            res = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(attn_mask == 0, float(0.0))
            return res
