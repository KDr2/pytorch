...
class FxirTestCase(InductorTestCase):
    ...
    def test_fallback_tuple_constant_arg(self):
        ...
        permute_node = next(
            iter(gm.graph.find_nodes(op="call_function", target=torch.ops.aten.permute.default)
        ))
        ...
        self.assertIsInstance(
            perm_arg, tuple, "Permutation argument should be a tuple"
        )
        self.assertEqual(len(perm_arg), 3)
        self.assertEqual(
            tuple(perm_arg), (0, 2, 1)
        )
