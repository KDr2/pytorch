...
class DeferredRecordingTest(TestMixin, TestCase):
    ...
    @set_caching_module_enabled(True)
    def test_deferred_recording_complete_caches_result(self) -> None:
        """Test that DeferredRecording.complete() caches the result.

        Verifies that when an encoder returns a DeferredRecording and
        complete() is called later, the result is cached properly.
        """
        ...
        # Execute: complete the deferred recording
        self.assertIsNotNone(deferred_obj)
        deferred_obj.complete(100)  # Cache encoded value 100
        ...
        def future_encoder(*args: object, **kwargs: object) -> object:
            def encode(future_result: Future[int]) -> interfaces.DeferredRecording:
                ...
                def on_complete(completed_future: Future[int]) -> None:
                    ...
                    deferred.complete(actual_result)
    ...
    @set_caching_module_enabled(True)
    def test_deferred_recording_complete_without_callback(self) -> None:
        """Test that complete() is safe when no callback is set.

        Verifies that calling complete() on a DeferredRecording without
        a registered callback does not raise an error.
        """
        # Setup: create a standalone DeferredRecording
        deferred = interfaces.DeferredRecording()

        # Execute & Assert: complete should not raise
        deferred.complete(42)  # Should do nothing but not raise
    ...
    def test_persistent_memoizer_memoize_with_deferred(self) -> None:
        ...
        deferred_objects[0].complete(10)
    ...
    def test_deferred_recording_multiple_calls_different_keys(self) -> None:
        ...
        deferred_objects[15].complete(30)
        deferred_objects[5].complete(10)
        deferred_objects[10].complete(20)
