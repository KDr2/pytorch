...
class AOTInductorTestsTemplate:
    ...
    def test_update_user_managed_buffer(self):
        ...
        mem_before, total = torch.cuda.mem_get_info()

        # Default behavior of updating buffer, should decrease free memory.
        runner.update_constant_buffer(new_weights, True, False, False)
        mem_after, _ = torch.cuda.mem_get_info()
        ...
        # Inplace substitube tensor, with default behavior, result should be different.
        ...
        new_weights = {
            "L__self___weight": torch.zeros(N, K, device=self.device),
            "L__self___bias": torch.ones(N, device=self.device),
        }
        ...
        runner.swap_constant_buffer()
        # EW = runner.extract_constants_map(False)
        print("BEFORE RUN!")
        breakpoint()
        new_output = runner_call(test_inputs)
        print("FINISH RUN!")
