"""E2E test for TPU Pallas codegen.

Compiles (a + b) * 2.0 with TPUTensor inputs via torch.compile with
pallas_tpu_native=True and asserts the generated wrapper code matches
...
# Topologically Sorted Source Nodes: [ret, ret_1], Original ATen: [aten.add, aten.mul]
...
class TestPallasTPUCodegen(unittest.TestCase):
    ...
    def test_codegen_matches_expected(self):
        saved = {
            "cpu_backend": inductor_config.cpu_backend,
            "pallas_tpu_native": inductor_config.pallas_tpu_native,
            "fx_graph_cache": inductor_config.fx_graph_cache,
        }
        inductor_config.cpu_backend = "pallas"
        inductor_config.pallas_tpu_native = True
        ...
        try:
            ...
            self.assertEqual(x._jax_array.devices(), {tpu_device})
            self.assertEqual(y._jax_array.devices(), {tpu_device})
            ...
            self.assertEqual(result._jax_array.devices(), {tpu_device})
            ...
            self.assertEqual(code, EXPECTED_WRAPPER_CODE)
        finally:
            inductor_config.cpu_backend = saved["cpu_backend"]
            inductor_config.pallas_tpu_native = saved["pallas_tpu_native"]
            inductor_config.fx_graph_cache = saved["fx_graph_cache"]
            torch._dynamo.reset()
