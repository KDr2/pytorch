...
class A(nn.Module):
    def __init__(self, dim: int, addend: torch.Tensor, subtrahend: torch.Tensor) -> None:
        super().__init__()
...
def _discover_ddp_ignored_params(module: torch.nn.Module, prefix: str):
    ...
    if isinstance(module, FSDP2):
        ddp_ignore_parameters =  [
            name for name, _ in module.named_parameters(prefix)
        ]
    else:
...
def _modify_ddp_ignored_params(ddp_ignored_param_names: list[str],
...
def _discover_fsdp_ignored_params(module: torch.nn.Module, ignored_path, path: str) -> set[torch.nn.Parameter]:
    ...
    if ignored_path == path:
        ...
        for name, sub_module in list(module.named_children()):
            ...
            child_ignored_params = _discover_fsdp_ignored_params(sub_module, ignored_path, child_path)
            total_ignored_params = total_ignored_params | child_ignored_params

    return total_ignored_params


def _post_order_wrap_fsdp(module: torch.nn.Module, mesh, path:str, ignored_path:str, ignored_params:set[torch.nn.Parameter])->torch.nn.Module:

    if ignored_path != path:
        for name, sub_module in list(module.named_children()):
            child_path = _append_prefix(path, name)
            _post_order_wrap_fsdp(sub_module, mesh, child_path, ignored_path, ignored_params)
...
class TestFullyShardIgnoreParams(FSDPTest):
    ...
    def test_ddp_A_fsdp_B_ddp_C(self):
        ...
        fsdp_ignored_params = _discover_fsdp_ignored_params(test_model, ignored_path=ignored_path, path="")
        test_model.module_b = _post_order_wrap_fsdp(test_model.module_b,
                                                    mesh=mesh,
                                                    path="module_b",
        ...
        self.assertEqual(fsdped_modules, set(['module_b', 'module_b.lin_b']))

        ddp_ignored_param_names = _discover_ddp_ignored_params(test_model, "")
        self.assertEqual(set(ddp_ignored_param_names), set(['module_b.subtrahend', 'module_b.lin_b.weight', 'module_b.lin_b.bias', 'module_b.module_c.lin_c.weight', 'module_b.module_c.lin_c.bias']))

        modified_ddp_ignored_param_names = _modify_ddp_ignored_params(ddp_ignored_param_names, fsdp_ignored_params, test_name_to_param_map)
        self.assertEqual(set(modified_ddp_ignored_param_names), set(['module_b.subtrahend', 'module_b.lin_b.weight', 'module_b.lin_b.bias']))

        DDP._set_params_and_buffers_to_ignore_for_model(module=test_model,
