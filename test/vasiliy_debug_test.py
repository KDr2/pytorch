...
def _test_impl(m: torch.nn.Module, args, num_subgraphs=1, skip_log_validation=False):
    ...
    torch._inductor.config.pre_grad_custom_pass = \
        lambda g: debug_linears_for_float8(g, target_folder)

    # run the extraction
    m = torch.compile(m)
    out = m(*args)

    if skip_log_validation:
        return
...
def test_placeholder_linear():
    ...
    _test_impl(m, (x,), skip_log_validation=True)
