...
"""
    ...
    def test_graph_make_graphed_callables(
        ...
        class ParameterlessModule(torch.nn.Module):
            def forward(self, x):
                idx = torch.arange(x.size(0), device=x.device).view(-1, 1).repeat(1, x.size(1))
        ...
        for _ in range(2):
            ...
            models.append(torch.nn.Sequential(model_section1, model_section2, model_section3))
        ...
        with torch.cuda.amp.autocast(with_amp, cache_enabled=cache_enabled):
            (
                ...
                (model_graphed[0], model_graphed[1], model_graphed[2], relu_control, loss_fn_control),
                (
