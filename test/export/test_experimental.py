...
from torch.export.experimental import _export_forward_backward, _sticky_export, _undo_sticky_export
...
def forward(self, p_linear_weight, p_linear_bias, c_lifted_tensor_0, x):
    ...
    def test_sticky_export(self):
        ...
        p = Pipeline(Model())
        p.model = sticky_export(p.model)
        res = p.generate(inp)

        p.model = undo_sticky_export(p.model) 
        res2 = p.generate(inp)
    ...
    def test_sticky_export_nested_inp(self):
        ...
        p = Pipeline(Model())
        p.model = _sticky_export(p.model)
        res = p.generate(input_tensor=inp, input_tensor2=inp2)

        p.model = _undo_sticky_export(p.model) 
