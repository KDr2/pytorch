...
class DecompOneOffTests(TestCase):
    ...
    def test_sdpa(self, device):
        ...
        class ScaledDotProductAttention(torch.nn.Module):
            ...
            def forward(self, query_layer, key_layer, value_layer, mask = None):
                attn_output = F.scaled_dot_product_attention(
                    query_layer, key_layer, value_layer, attn_mask=mask, dropout_p=0.0, is_causal=True
                )
                return attn_output
        ...
        masks = [torch.randn(1, 1, 100, 100, device=device), torch.randn(100, 100, device=device),]
        ...
        for mask in masks:
            ...
            compiled_res = fx_g(query_layer, key_layer, value_layer, mask)
