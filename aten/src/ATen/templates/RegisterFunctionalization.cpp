...
static inline Tensor to_meta(const Tensor& t) {
...
static inline std::optional<Tensor> to_meta(const std::optional<Tensor>& t) {
  if (t.has_value()) {
    return to_meta(*t);
  }
  return std::nullopt;
...
static inline std::vector<Tensor> to_meta(at::ITensorListRef t_list) {
...
static inline c10::List<::std::optional<Tensor>> to_meta(const c10::List<::std::optional<Tensor>>& t_list) {
