...
Tensor embedding_backward_cuda_kernel(
  ...
  AT_DISPATCH_INDEX_TYPES(orig_indices.scalar_type(), "embedding_backward_cuda_kernel", [&] () {
    ...
    const int64_t min_blocks_for_good_occupancy = 1024;  // Tunable threshold
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16,
      grad.scalar_type(), "embedding_bag_backward_cuda_compute_grad_weight", [&] {
        ...
        if (use_fused_kernel) {
          ...
          // Launch fused kernel - grid size is based on max_partial_segment, not max_segments
          compute_grad_weight_atomic_accumulate<scalar_t><<<grid, block, 0, stream>>>(
          ...
          // Original two-pass path: compute_grad_weight + sum_and_scatter
          TensorOptions op;
