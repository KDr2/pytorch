...
(const Tensor& grad_output,
 ...
 const Tensor& output) {
  ...
  @autoreleasepool {
    string key =
        "leaky_relu_backward" + getTensorsStringKey({self, grad_output}) + ":" + std::to_string(negative_slope.to<double>());
    auto cachedGraph = LookUpOrCreateCachedGraph<CachedGraph>(key, [&](auto mpsGraph, auto newCachedGraph) {
      MPSGraphTensor* inputTensor = mpsGraphRankedPlaceHolder(mpsGraph, self);
...
Tensor& hardtanh_backward_out_mps(const Tensor& grad_output,
  ...
  @autoreleasepool {
    string key = "hardtanh_backward_out_mps:" + getTensorsStringKey({grad_output}) + ":" + std::to_string(min.to<double>()) +
        ":" + std::to_string(max.to<double>());
