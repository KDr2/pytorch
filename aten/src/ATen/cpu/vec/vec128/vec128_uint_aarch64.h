...
VEC_UINT_NEON_TEMPLATE(2, 64)
VEC_UINT_NEON_TEMPLATE(4, 32)
VEC_UINT_NEON_TEMPLATE(8, 16)
...
template <>
inline Vectorized<uint64_t> operator~(const Vectorized<uint64_t>& a) {
  uint64x2_t val = a;
  return ~val;
}

template <>
inline Vectorized<uint32_t> operator~(const Vectorized<uint32_t>& a) {
  return vmvnq_u32(a);
}

template <>
inline Vectorized<uint16_t> operator~(const Vectorized<uint16_t>& a) {
  return vmvnq_u16(a);
}
...
VEC_UINT_NEON_OPS(2, 64)
VEC_UINT_NEON_OPS(4, 32)
VEC_UINT_NEON_OPS(8, 16)
...
inline Vectorized<uint64_t> Vectorized<uint64_t>::arange(
  ...
  const uint64x2_t step_sizes = {0, 1};
...
inline Vectorized<uint32_t> Vectorized<uint32_t>::arange(
  ...
  const uint32x4_t step_sizes = {0, 1, 2, 3};
...
inline Vectorized<uint16_t> Vectorized<uint16_t>::arange(
  ...
  const uint16x8_t step_sizes = {0, 1, 2, 3, 4, 5, 6, 7};
...
Vectorized<uint64_t> inline operator>>(
  ...
  uint64x2_t x = a;
  uint64x2_t y = b;
  uint64x2_t z = {std::min(y[0], (uint64_t)64), std::min(y[1], (uint64_t)64)};
  return x >> z;
}
...
Vectorized<uint32_t> inline operator>>(
  ...
  uint32x4_t z = vminq_u32(b, bound);
  return x >> z;
}
...
Vectorized<uint16_t> inline operator>>(
  ...
  uint16x8_t z = vminq_u16(b, bound);
  return x >> z;
}
...
Vectorized<uint64_t> inline operator<<(
  ...
  uint64x2_t y = b;
  uint64x2_t z = {std::min(y[0], (uint64_t)64), std::min(y[1], (uint64_t)64)};
  return vshlq_u64(a, vreinterpretq_s64_u64(z));
}
...
Vectorized<uint32_t> inline operator<<(
  ...
  uint32x4_t bound = vdupq_n_u32(32);
  uint32x4_t z = vminq_u32(b, bound);
  return vshlq_u32(a, vreinterpretq_s32_u32(z));
}
...
Vectorized<uint16_t> inline operator<<(
  ...
  uint16x8_t bound = vdupq_n_u16(16);
  uint16x8_t z = vminq_u16(b, bound);
  return vshlq_u16(a, vreinterpretq_s16_u16(z));
}
...
inline Vectorized<uint64_t> Vectorized<uint64_t>::set(
    const Vectorized<uint64_t>& a,
    const Vectorized<uint64_t>& b,
    uint64_t count) {
  if (count == 0) {
    ...
    uint64x2_t c = {b.values[0], a.values[1]};
    return c;
  }
}

inline Vectorized<uint32_t> Vectorized<uint32_t>::set(
    const Vectorized<uint32_t>& a,
    const Vectorized<uint32_t>& b,
    uint64_t count) {
  if (count == 0) {
...
inline Vectorized<uint16_t> Vectorized<uint16_t>::set(
    const Vectorized<uint16_t>& a,
    const Vectorized<uint16_t>& b,
    uint64_t count) {
  if (count == 0) {
...
Vectorized<uint16_t> inline operator/(
  ...
  uint32x4_t highBitsResult = highBitsA / highBitsB;
  uint32x4_t lowBitsResult = lowBitsA / lowBitsB;
  return vuzp1q_u16(
      vreinterpretq_u16_u32(lowBitsResult),
      vreinterpretq_u16_u32(highBitsResult));
}
...
Vectorized<uint8_t> inline operator/(
  ...
  uint16x8_t highBitsResult = highBitsA / highBitsB;
  uint16x8_t lowBitsResult = lowBitsA / lowBitsB;
  return vuzp1q_u8(
      vreinterpretq_u8_u16(lowBitsResult),
      vreinterpretq_u8_u16(highBitsResult));
}
