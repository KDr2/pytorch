...
inline void convertImpl(
  ...
  for (uint64_t i = 0; i < len; i++) {
    from_type tmp0 = static_cast<from_type>(0);
    __builtin_memcpy(&tmp0, src + i, sizeof(from_type));
    to_type tmp1 = static_cast<to_type>(tmp0);
    __builtin_memcpy(dst + i, &tmp1, sizeof(to_type));
  }
}
